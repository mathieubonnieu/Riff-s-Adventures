...
public class SnakeControlScript : MonoBehaviour
{
    ...
    private void STATUS ()
    {
        ...
        if(_Animator.GetCurrentAnimatorStateInfo(0).IsTag("ThrowUp"))
        {
            _Status["ThrowUp"] = true;
        }
        else if(!_Animator.GetCurrentAnimatorStateInfo(0).IsTag("ThrowUp"))
        {
            _Status["ThrowUp"] = false;
        }
    ...
    //--------------------------------------------------------------------- EYE STATUS
    private void EYE_STATUS ()
    {
        if(Input.GetKeyDown(KeyCode.W))
        {
            if(_Eye_Status)
            {
                _Animator.CrossFade("eye2", 0.3f, 1, 0);
                _Eye_Status = false;
            }
            else if(!_Eye_Status)
            {
                _Animator.CrossFade("eye1", 0.3f, 1, 0);
                _Eye_Status = true;
            }
        }
    }
    ...
    private void EAT ()
	{
		...
		{
	    	_Animator.CrossFade("hold", 0.2f, 0, 0);
	    }
        if (_Animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1
            && _Animator.GetCurrentAnimatorStateInfo(0).IsName("hold")
            && !_Animator.IsInTransition(0))
        {
            _Animator.CrossFade("eat", 0.1f, 0, 0);
        }
        if (_Animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1
    ...
    private void THROWUP ()
	{
		...
		{
	    	_Animator.CrossFade("throwup", 0.2f, 0, 0);
	    }
        if (_Animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1
            && _Animator.GetCurrentAnimatorStateInfo(0).IsName("throwup")
            && !_Animator.IsInTransition(0))
        {
            _Animator.CrossFade("idle", 0.1f, 0, 0);
        }
