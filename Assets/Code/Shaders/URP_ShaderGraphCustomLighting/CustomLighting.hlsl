...
float ToonAttenuationLamber(int lightIndex, float3 positionWS, float pointBands, float spotBands, float3 normalWS){
	#if !USE_FORWARD_PLUS
		lightIndex = GetPerObjectLightIndex(lightIndex);
	#endif
	#if USE_STRUCTURED_BUFFER_FOR_LIGHT_DATA
	...
	float3 lightDirection = normalize(lightVector);  // Direction from light to the point
	float lambert = LightingLambert(half3(1,1,1), lightDirection, normalWS);  // Apply steps for Lambertian light
	
	// Apply steps for Lambertian light
	// float lambertStep = step(0.01, lambert);  // Add a step for the Lambertian light

	// Spot
	half3 spotDirectionNormalized = half3(spotDirection.xyz);
	half SdotL = dot(spotDirectionNormalized, lightDirection);
	half spotAtten = saturate(SdotL * distanceAndSpotAttenuation.z + distanceAndSpotAttenuation.w);
	spotAtten *= spotAtten;
	float maskSpotToRange = step(dist, 1);

	// Attenuation * saturate(floor(lambert * pointBands) / pointBands)
	bool isSpot = (distanceAndSpotAttenuation.z > 0);
	float atten = isSpot ? floor(spotAtten * spotBands) : ceil((1 - dist) * pointBands);
	float bands = isSpot ? spotBands : pointBands;
	float lambertStep = floor((1 - lambert) * (bands - 1));
	// lambertStep = min(lambertStep, atten - 1);
	atten = (lambertStep == bands - 1 || atten <= 0) ? 0 : max(atten - lambertStep, 1);
	
	return isSpot ? saturate(atten / bands) : saturate(atten / bands);
}
	...
	LIGHT_LOOP_BEGIN(pixelLightCount)
	...
	#endif
		{
			if (PointLightBands <= 1 && SpotLightBands <= 1){
				...
				// Multiple bands
				// diffuseColor += light.direction, WorldNormal
				diffuseColor += light.color * light.shadowAttenuation * ToonAttenuationLamber(lightIndex, WorldPosition, PointLightBands, SpotLightBands, WorldNormal);
				float spec = LightingSpecular(light.color, light.direction, WorldNormal, WorldView, float4(SpecColor, 0), Smoothness);
				spec = (floor(spec * (PointLightBands - 1))) / (PointLightBands - 1);
				specularColor += saturate(spec) * light.color;
			}
		}
	LIGHT_LOOP_END
#endif
